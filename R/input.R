...
process_inputs <- function(formulas, pars, family, link,
  ...
  link <- causl::link_setup(link,
    family = family[-(5)], vars = LHSs,
  ...
  if (method == "inversion" || method == "bootstrap" || method == "rejection") {
    tmp <- causl::pair_copula_setup(
      formulas = formulas[[5]], family = family[[5]], pars = pars[[kwd]],
  ...
  if (!(is.null(dat)) && is.null(qtls)) {
    ...
    wh_q <- names(dat)
    qtls <- process_prespecified(dat,
      prespec = wh_q, cond = control$pm_cond,
  ...
  out <- list(
    formulas = formulas, pars = pars, family = family, link = link,
    LHSs = list(LHS_C = LHS_C, LHS_Z = LHS_Z, LHS_X = LHS_X, LHS_Y = LHS_Y),
  ...
  if (method == "bootstrap") {
    ...
    if (is.null(risk_form)) {
      stop("Must specify risk formula in copula formulas.
    ...
    if (is.null(pars[[cns_kwd]])) {
      if (length(cenY) == 1) {
        ...
        if (any(has_all)) {
          ...
          stop("Censoring distribution not inferable from input")
        }
      }
    }
