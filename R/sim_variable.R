  ...
  if (k > 0) {
    ...
    for (j in rev(0:(k - 1))) {
      for (i in rev(seq_len(p)[-1])) {
        ...
        idxs <- c(i, (j + 1))
        form <- do.call("[[", c(list(formulas), as.list(idxs)))
  ...
  for (j in rev(0:k)) { # in paper use k+1, we're not so go from 1, k
    if (j > 0) {
      ...
      for (i in rev(seq_len(p)[-1])) {
        ...
        if (j == 0) {
          insert_col <- paste0(
            vnm, "|", paste0(time_vars[1:(i - 1)], collapse = ""),
            "_", j
          )
        } else {
          insert_col <- paste0(
