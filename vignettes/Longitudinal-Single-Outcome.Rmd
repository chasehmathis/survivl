...
fams <- list(list(5, 4),
             ...
             5,
             2)
causal_parameters <- c(-0.5, 0.1, 0.02, -0.5)
pars <- list(
  B = list(beta = 0.1), 
  C = list(beta = 1),
  ...
  cop = list(Y = list(L = list(k_tau = 0.5, par2 = 5))))
...
Now, we choose to use three estimators to recover our causal parameters: inverse probability weighting (IPW), outcome regression, and a naive (unadjusted) model.

## IPW Estimator

Since $A_1 = A_2 = A_0$, only $A_0$, $A_3$, and $A_4$ contribute non-trivial weights.
...
# Compute weights depending on actual values of A_t
...
The outcome model directly adjusts for the time-varying confounders $L_0, \ldots, L_4$.

```{r outcome}
glm_Y_outcome <- glm(Y ~ B + C + L_0 + L_1 + L_2 + L_3 + L_4
...
The naive model ignores confounding by the time-varying $L$.
...
We compare the three estimators against the true causal parameters. Each value is the number of standard errors from the truth — we expect this to be less than 2 about 95% of the time.
...
shared_nms <- rownames(sumY_ipw$coefficients)  # (Intercept), B, C, I(sum A)

cat("IPW:\n")
print(round(abs(sumY_ipw$coefficients[, 1] - causal_parameters) / sumY_ipw$coefficients[, 2], 3))

cat("\nOutcome model:\n")
out_coef <- sumY_outcome$coefficients[shared_nms, ]
print(round(abs(out_coef[, 1] - causal_parameters) / out_coef[, 2], 3))

cat("\nNaive:\n")
print(round(abs(sumY_naive$coefficients[, 1] - causal_parameters) / sumY_naive$coefficients[, 2], 3))
```
